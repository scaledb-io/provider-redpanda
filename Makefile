...
IMG ?= ghcr.io/scaledb-io/provider-redpanda-dev:latest
...
CHART_DIR ?= charts/provider-redpanda
...
helm-install: ## Install the provider using Helm.
	helm install provider-redpanda $(CHART_DIR) --create-namespace

.PHONY: helm-upgrade
helm-upgrade: ## Upgrade the provider using Helm.
	helm upgrade provider-redpanda $(CHART_DIR)

.PHONY: helm-uninstall
helm-uninstall: ## Uninstall the provider using Helm.
	helm uninstall provider-redpanda

.PHONY: helm-template
helm-template: ## Render Helm chart templates locally (dry-run).
	helm template provider-redpanda $(CHART_DIR)
